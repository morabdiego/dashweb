...
def render_selector(
    title, 
    options, 
    displayed_value,  # Now we use displayed_value for what's shown in UI 
    on_select, 
    displayed_start_date,  # This is what's shown in UI
    on_start_date_change,
    displayed_end_date,  # This is what's shown in UI
    on_end_date_change,
    on_apply_changes,
    placeholder="Select...",
    has_pending_changes=None  # Cambiado a Var compatible con Reflex
):
    """
...
def handle_selector_logic(
    ...
    """
    # Las funciones de evento en Reflex deben devolver eventos de Reflex válidos
    # o ser funciones de evento decoradas con @rx.event
    
    # Para garantizar compatibilidad, retornamos directamente las 
    # funciones del estado que ya están decoradas con @rx.event
    return {
